...
void *file_read(void *arg)
{
     ...
     int cSock;
     char directory[60];
     strcpy(directory,"/home/lee/adv-sys-programming/file_");
     cSock = *((int*)arg);
     sprintf(intoString,"%d",cSock);
     strcat(directory,intoString);
     printf("%s",directory); 
     pFile = fopen(directory,"r");
     while(!feof(pFile)){
	fgets(str_read,MAX_STR_SIZE,pFile);
        str_read[strlen(str_read)-1]='\n';
        printf("%s",str_read);
        send(cSock,str_read,strlen(str_read),0);
     }
}
...
launch_chat(void)
{
    ...
    while (1) { //client`s main loop
        ...
        if (FD_ISSET(clientSock, &rfds)) {
            ...
            printf("%s\n",rdata);
            threadId=pthread_create(&sockThreads[0],NULL,file_read,(void*)&a);
	    if(threadId<0)
	    {
		...
		exit(1);
            ...
            pthread_join(sockThreads[0],(void**)&status);
            printf("return thread 0 %d\n",status);
	    
	    threadId=pthread_create(&sockThreads[1],NULL,file_read,(void*)&b);
            if(threadId<0)
	    {
		perror("thread create error");
		exit(1);
            ...
            pthread_join(sockThreads[1],(void**)&status);
            printf("return thread 1 %d\n",status);
            
	    threadId=pthread_create(&sockThreads[2],NULL,file_read,(void*)&c);
	    if(threadId<0)
	    {
		perror("thread create error");
		exit(1);
            ...
            pthread_join(sockThreads[2],(void**)&status);
